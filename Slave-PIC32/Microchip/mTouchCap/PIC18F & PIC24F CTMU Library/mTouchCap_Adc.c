...
{	
	...
	//************************************/
...
#ifdef __PIC24F__	
		#ifdef ADC_LEGACY
		Adc_Port_Configuration(ADC_PORT_CONFIG_DEFAULT_VALUE);	// A/D mux must connect to channel for CTMU to drain charge
		#endif
		...
		#endif	
#else
		AD_CON_REG0 = ADC_CONFIG_ADCON0;
		AD_CON_REG1 = ADC_CONFIG_ADCON1;
		Adc_Port_Configuration1(ADC_PORT_CONFIG_DEFAULT_VALUE);	// A/D mux must connect to channel for CTMU to drain charge
		Adc_Port_Configuration2(ADC_PORT_CONFIG_DEFAULT_VALUE);	// A/D mux must connect to channel for CTMU to drain charge
